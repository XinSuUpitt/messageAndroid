...
import com.messagecube.messaging.BuildConfig;
...
import com.messagecube.messaging.datamodel.binding.ImmutableBindingRef;
...
import com.messagecube.messaging.datamodel.data.ConversationData.SimpleConversationDataListener;
import com.messagecube.messaging.datamodel.data.DraftMessageData;
import com.messagecube.messaging.datamodel.data.DraftMessageData.CheckDraftForSendTask;
import com.messagecube.messaging.datamodel.data.DraftMessageData.CheckDraftTaskCallback;
import com.messagecube.messaging.datamodel.data.DraftMessageData.DraftMessageDataListener;
...
import com.messagecube.messaging.datamodel.data.PendingAttachmentData;
import com.messagecube.messaging.datamodel.data.SubscriptionListData.SubscriptionListEntry;
import com.messagecube.messaging.sms.MmsConfig;
import com.messagecube.messaging.ui.AttachmentPreview;
import com.messagecube.messaging.ui.BugleActionBarActivity;
import com.messagecube.messaging.ui.PlainTextEditText;
import com.messagecube.messaging.ui.conversation.ConversationInputManager.ConversationInputSink;
import com.messagecube.messaging.ui.mediapicker.AudioRecordView;
...
import com.messagecube.messaging.util.BuglePrefs;
import com.messagecube.messaging.util.ContentType;
import com.messagecube.messaging.util.LogUtil;
import com.messagecube.messaging.util.MediaUtil;
...
import mobi.messagecube.sdk.openAPI.MyLocation;
...
public class ComposeMessageView extends LinearLayout
 ...
 //   private ImageButton mAttachMediaButton1;
    private ImageButton mSearchButton;
    private ImageButton mWebSearchLinearLayoutCloseButton;
    ...
    BroadcastReceiver mMessageReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            sendMessageInternalStep2(true, forwardMessage);
        }
    };
    ...
    public void setmLocationEnabled(boolean enable) {
        mLocationEnabled = enable;
    }

    public void setCurrAddress(String string) {
        currAddress = "" + string;
        System.out.println("currAddress is " + currAddress);
        if (!currAddress.equals("") && mComposeEditText != null) {
            //mComposeEditText.setText("Current Location is: Latitude - "+ mLatitude + ", Longitude - " + mLongitude);
            mComposeEditText.setText("My current location is: " + currAddress);
        }
    }
    ...
    // TODO: Setup LocalBroadcastManager to receive messages
    @Override
    protected void onAttachedToWindow() {
        super.onAttachedToWindow();
        LocalBroadcastManager.getInstance(mOriginalContext).registerReceiver(mMessageReceiver,
    ...
    protected void onDetachedFromWindow() {
        ...
        LocalBroadcastManager.getInstance(mOriginalContext).unregisterReceiver(mMessageReceiver);
    }
    ...
    protected void onFinishInflate() {
        ...
        mSendButton.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(final View clickView) {
                LinearLayout tmp = (LinearLayout) findViewById(R.id.compose_search_result_linearlayout);
        ...
        mAttachMediaButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(final View clickView) {
             ...
             //   mMediaPicker.onCreateView()
            }
        });

    //    mAttachMediaButton1 = (ImageButton) findViewById(R.id.attach_media_button1);
     //   mAttachMediaButton1.setOnClickListener(new View.OnClickListener() {
     //       @Override
     //       public void onClick(final View clickView) {
                // Showing the media picker is treated as starting to compose the message.
      //          mInputManager.showHideMediaPicker(true /* show */, true /* animate */);
      ...
      //  });
        ...
        mSearchButton = (ImageButton) findViewById(R.id.bing_search_trigger_button);
            ...
            public void onClick(View v) {
                ...
                if (isTypingSearch) {
                    ...
                    */
                    ...
                    imm.showSoftInput(mComposeEditText,InputMethodManager.SHOW_FORCED);
                    mComposeEditText.setText("Search: ");
        ...
        mWebSearchLinearLayoutCloseButton = (ImageButton) findViewById(R.id.search_close_button);
        mWebSearchLinearLayoutCloseButton.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
                LinearLayout tmp = (LinearLayout) findViewById(R.id.compose_search_result_linearlayout);
                tmp.setVisibility(GONE);
    ...
    private void loadSearchResult() {
        ...
        // TODO: Integrate MessageCube SDK to send a message
        String messageToSend = mComposeEditText.getText().toString();

        // Step 1: Check if it's a format that MessageCube should process.
        if (MessageItemParser.isSimpleWeb(messageToSend)) {
            // Step 2:
            final Context context = Factory.get().getApplicationContext();

            // Step 3: Get keyword
            final String keyword = MessageItemParser.getSearchKeywordFromText(messageToSend);
            ...
            if (mLocationEnabled) {
                ...
                try {
                    List<Address> addesses = geocoder.getFromLocation(mLatitude, mLongitude, 1);
                    if (addesses.size() == 1) {
                        postalCode = addesses.get(0).getPostalCode();
                    }
                } catch (Exception e) {
            ...
            MessageCubeParams messageCubeParams = new MessageCubeParams(context, keyword, postalCode, null, null, null, -1);

            // We have to make a local pointer to the
            final ComposeMessageView pointer = this;
            ...
            ReceiveProcess receiveProcess = new ReceiveProcess() {
                ...
                public void onSuccess(String textBody, ArrayList<BaseClass> baseClasses) {
                    // Have to create a final string which setMessageText needs.

                    // TODO: Text is a complicated MessageCube message format.

                    MessageCubeParams messageCubeParams = new MessageCubeParams(pointer.getContext(), keyword, finalPostalCode, null, null, null, -1);
                    ReceiveProcess receiveProcess = new ReceiveProcess() {
                        @Override
    ...
    private void sendMessageInternal(final boolean checkMessageSize) {
        ...
        if (mHost.isReadyForAction()) {
            ...
            // TODO: Set the compose edit text to ""
            ...
            if (MessageItemParser.isSimpleWeb(messageToSend)) {
                ...
                // Step 3: Get keyword
                String keyword = MessageItemParser.getSearchKeywordFromText(messageToSend);

                // Do something for a debug build
                if (!BuildConfig.DEBUG) {
                    // Track the keyword.
                    MixpanelAPI mixpanel = MixpanelAPI.getInstance(context, "1f9b4107070551d59f285c8cf2195ccd");
                    try {
                ...
                ReceiveProcess receiveProcess = new ReceiveProcess() {
                    ...
                    public void onSuccess(String textBody, ArrayList<BaseClass> baseClasses) {
                        ...
                        // Step 6: Send message before load
                        pointer.post(new Runnable() {
                            @Override
                            public void run() {
                                // Move this to where the user just clicks the send button.
                                // sendMessageInternalStep2(checkMessageSize, finalTextBody);
                                // TODO: Try to fix unexpected message_title_manual_download messages.
                                sendMessageInternalStep2(false, finalTextBody);
                            }
                        });
                    }
                };
                ...
                sendMessageInternalStep2(checkMessageSize, finalMessageToSend);
            }
        } else {
    ...
    // Originally it's a part of sendMessageInternal().
    // After MessageCube SDK is added, we extract this part out of it.
    // Must be on the main thread!
    private void sendMessageInternalStep2(final boolean checkMessageSize, final String messageToSend) {
    ...
    private void setSendWidgetAccessibilityTraversalOrder(final int mode) {
        if (OsUtil.isAtLeastL_MR1()) {
          ...
          //  mAttachMediaButton1.setAccessibilityTraversalBefore(R.id.compose_);
            switch (mode) {
                case SEND_WIDGET_MODE_SIM_SELECTOR:
    ...
    public void setAccessibility(boolean enabled) {
        ...
        //    mAttachMediaButton1.setImportantForAccessibility(IMPORTANT_FOR_ACCESSIBILITY_YES);
        ...
        //    mAttachMediaButton1.setImportantForAccessibility(IMPORTANT_FOR_ACCESSIBILITY_NO);
        }
    }
}
