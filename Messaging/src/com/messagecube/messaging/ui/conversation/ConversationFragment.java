...
import com.messagecube.messaging.datamodel.MessagingContentProvider;
import com.messagecube.messaging.datamodel.action.InsertNewMessageAction;
...
import com.messagecube.messaging.datamodel.data.ConversationParticipantsData;
...
import com.messagecube.messaging.ui.SnackBar;
import com.messagecube.messaging.ui.UIIntents;
import com.messagecube.messaging.ui.animation.PopupTransitionAnimation;
import com.messagecube.messaging.ui.contact.AddContactsConfirmationDialog;
import com.messagecube.messaging.ui.conversation.ComposeMessageView.IComposeMessageViewHost;
import com.messagecube.messaging.ui.conversation.ConversationInputManager.ConversationInputHost;
import com.messagecube.messaging.ui.conversation.ConversationMessageView.ConversationMessageViewHost;
import com.messagecube.messaging.ui.conversationlist.Background_Image;
import com.messagecube.messaging.ui.conversationlist.ConversationListActivity;
...
import com.messagecube.messaging.util.ChangeDefaultSmsAppHelper;
...
import com.messagecube.messaging.util.SafeAsyncTask;
import com.messagecube.messaging.util.TextUtil;
...
public class ConversationFragment extends Fragment implements ConversationDataListener,
        ...
        mAdapter = new ConversationMessageAdapter(getActivity(), null, this,
                ...
                new View.OnClickListener() {
                    ...
                    public void onClick(final View v) {
                        ...
                        System.out.println("message data is: " + messageView.getData().getText().toString());
                    }
                },
    ...
    public View onCreateView(final LayoutInflater inflater, final ViewGroup container,
        ...
        SharedPreferences sharedPreferences = getActivity().getSharedPreferences(mConversationId + ConversationListActivity.BACKGROUNDIMAGE_FILENAME, Context.MODE_PRIVATE);
        int drawable = sharedPreferences.getInt(ConversationListActivity.BACKGROUNDIMAGE, R.drawable.background_default);
        String img = sharedPreferences.getString(ConversationListActivity.BACKGROUNDIMAGE+"String", "");
        view.setBackgroundResource(drawable);
        System.out.println("Get from local + " + img);
        //// TODO: 10/2/17
        if (img.equals("")) {
            view.setBackgroundResource(drawable);
        } else {
            Drawable d = new BitmapDrawable(getResources(), BitmapFactory.decodeFile(img));
            view.setBackground(d);
        }

        System.out.println("Background image for conversation list is "+ drawable);
        ...
        String string1 = sharedPreferences1.getString("Latitude", "0.0");
        String string2 = sharedPreferences1.getString("Longitude", "0.0");
        String currLocation = sharedPreferences1.getString("currAddress", "");
        sharedPreferences1.edit().putString("Latitude", "" + 0.0).apply();
        sharedPreferences1.edit().putString("Longitude", "" + 0.0).apply();
        sharedPreferences1.edit().putString("currAddress", "").apply();
        double mLatitude = Double.parseDouble(string1);
        double mLongitude = Double.parseDouble(string2);
        if (mLatitude != 0.0 && mLongitude != 0.0) {

            mComposeMessageView.setText(currLocation);
        }
    ...
    private void scrollToPosition(final int targetPosition, final boolean smoothScroll) {
        if (smoothScroll) {
            ...
            // mRecyclerView.scrollToPosition(319);
    ...
    public void onResume() {
        ...
        if (mLatitude != 0.0 && mLongitude != 0.0) {
            ...
            System.out.println("Currlocation is "+ currLocation);

            mComposeMessageView.setText(currLocation);
        }

        SharedPreferences sharedPreferences2 = getActivity().getSharedPreferences("VcardInfo", Context.MODE_PRIVATE);
        String contactInfo = sharedPreferences2.getString("contactString", "");
        if (!contactInfo.equals("")) {
            sharedPreferences2.edit().putString("contactString", "").apply();
            mComposeMessageView.setText(contactInfo);
        }
    ...
    public boolean onOptionsItemSelected(final MenuItem item) {
        switch (item.getItemId()) {
            ...
            case R.id.change_background:
                onChangeBackground(mConversationId);
                return true;
    ...
    private void onChangeBackground(String messageId) {
        Intent intent = new Intent(getActivity(), Background_Image.class);
        intent.putExtra("messageIdForBackground", messageId);
