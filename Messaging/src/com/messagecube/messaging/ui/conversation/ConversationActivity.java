...
import com.messagecube.messaging.datamodel.BadgeUtils;
import com.messagecube.messaging.Factory;
import com.messagecube.messaging.ui.BugleActionBarActivity;
import com.messagecube.messaging.ui.ImeDetectFrameLayout;
import com.messagecube.messaging.ui.UIIntents;
import com.messagecube.messaging.ui.contact.ContactPickerFragment;
import com.messagecube.messaging.ui.contact.ContactPickerFragment.ContactPickerFragmentHost;
import com.messagecube.messaging.ui.conversation.ConversationActivityUiState.ConversationActivityUiStateHost;
import com.messagecube.messaging.ui.conversation.ConversationFragment.ConversationFragmentHost;
...
import static com.messagecube.messaging.ui.conversationlist.ConversationListActivity.BACKGROUNDIMAGE;
import static com.messagecube.messaging.ui.conversationlist.ConversationListActivity.BACKGROUNDIMAGE_FILENAME;
...
public class ConversationActivity extends BugleActionBarActivity
    ...
    protected void onCreate(final Bundle savedInstanceState) {
        ...
        drawable = getIntent().getIntExtra("BackGroundImage", 1111);
        drawableString = getIntent().getStringExtra("BackGroundImage" + "String");
        messageId = getIntent().getStringExtra("messageId");
        SharedPreferences sharedPreferences = getSharedPreferences(messageId + BACKGROUNDIMAGE_FILENAME, Context.MODE_PRIVATE);
        if (drawable != 1111) {
            sharedPreferences.edit().putInt(BACKGROUNDIMAGE, drawable).apply();
            sharedPreferences.edit().putString(BACKGROUNDIMAGE + "String", drawableString).apply();
        } else {
            sharedPreferences.edit().putString(BACKGROUNDIMAGE + "String", drawableString).apply();
        }

        ImeDetectFrameLayout imeDetectFrameLayout = (ImeDetectFrameLayout) findViewById(R.id.conversation_and_compose_container);
        ...
        if (mUiState == null) {
            ...
            if (conversationId == null && messageId != null) {
                mUiState = new ConversationActivityUiState(messageId);
            } else {
                mUiState = new ConversationActivityUiState(conversationId);
            }

            // TODO: Mark this conversion is read.
            BadgeUtils.removeConversationIdInBadgeInBackgroundThread(Factory.get().getApplicationContext(), mUiState.getConversationId());
        }
    ...
    private void updateUiState(final boolean animate) {
        ...
        //fragmentTransaction.setCustomAnimations(R.animator.right_slide_in, R.animator.left_slide_out);
        ...
        if (needConversationFragment) {
            ...
            if (conversationFragment == null) {
                ...
                fragmentTransaction.replace(R.id.conversation_fragment_container,
...
//        }
        System.out.println("slide from left");
        invalidateActionBar();
    }
